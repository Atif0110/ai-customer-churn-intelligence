...
def analyze_customer(v1: float, v2: float, v3: float):
    try:
        if not (0 <= v1 <= 500 and 0 <= v2 <= 50 and 0 <= v3 <= 120):
...
def single_analysis_page():
    ...
    if st.button("🔍 Analyze", use_container_width=True, key="analyze_btn"):
        ...
        if result:
            ...
            with sim_col1:
                change_pct = st.slider("Increase Usage by (%)", 0, 100, 10, step=5, key="sim_slider")
            with sim_col2:
                if st.button("📊 Simulate Impact", use_container_width=True, key="sim_btn"):
